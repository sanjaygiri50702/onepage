<?php
function onepage_add_service_section(){
    $options = onepage_get_theme_options();
    if(intval($options['service_section_enable'])!==1){
        return ;
    }
    $section_details = onepage_service_section_details();
    onepage_render_service_section($section_details);

}
function onepage_service_section_details(){
    $options = onepage_get_theme_options();
    $content['title'] = $options['service_title'];
    $content['description'] = $options['service_description'];
    return $content;
}
function onepage_render_service_section($content = array()){ ?>
    <div id="services" class="page-section relative">
        <div class="container"><!-- .container -->
            <div class="services-content-title clear">
                <header class="entry-header section-title">
                    <h2 class="entry-title"><?php echo $content['title'];?></h2>
                </header><!-- .entry-header -->

                <p class="description"><?php echo $content['description'];?></p>
            </div><!-- .services-contennt -->
            <div class="entry-content"><!-- .entry-content -->
                <div class="tabs-wrapper"><!-- .tabs-wrapper -->
                    <ul class="nav-tabs">
                        <li class="active">
                            <a href="#Branding">
                                <i class="fa fa-send-o"></i>
                                <h6>Branding</h6>
                                <span>1</span>
                            </a>
                        </li>
                        <li>
                            <a href="#web-design">
                                <i class="fa fa-television"></i>
                                <h6>Web Design</h6>
                                <span>2</span>
                            </a>
                        </li>
                        <li>
                            <a href="#multi-media">
                                <i class="fa fa-film"></i>
                                <h6>multi media</h6>
                                <span>3</span>
                            </a>
                        </li>
                        <li>
                            <a href="#Photography">
                                <i class="fa fa-camera-retro"></i>
                                <h6>Photography</h6>
                                <span>4</span>
                            </a>
                        </li>
                    </ul><!-- .nav-tabs -->

                    <div class="tabs-contents-wrapper"><!--.tabs-contents-wrapper-->
                        <div id="Branding" class="tab-pane active">
                            <strong class="pull-left">A brand is more than just a logo which communicates to your audience who your company is.</strong>

                            <div class="entry-summary pull-right">
                                <p>Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris. Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris.</p>
                            </div><!-- .entry-summary -->
                        </div><!-- .tab-pane -->

                        <div id="web-design" class="tab-pane">
                            <strong class="pull-left">A brand is more than just a logo which communicates to your audience who your company is.</strong>

                            <div class="entry-summary pull-right">
                                <p>Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris. Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris.</p>
                            </div><!-- .entry-summary -->
                        </div><!-- .tab-pane -->

                        <div id="multi-media" class="tab-pane">
                            <strong class="pull-left">A brand is more than just a logo which communicates to your audience who your company is.</strong>

                            <div class="entry-summary pull-right">
                                <p>Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris. Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris.</p>
                            </div><!-- .entry-summary -->
                        </div><!-- .tab-pane -->

                        <div id="Photography" class="tab-pane">
                            <strong class="pull-left">A brand is more than just a logo which communicates to your audience who your company is.</strong>

                            <div class="entry-summary pull-right">
                                <p>Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris. Mauris ut metus a nibh fermentum mollis. Cras ullamcorper ac lectus eget hendrerit. Aenean lacinia volutpat blandit. In ultricies iaculis orci queleifend. Nunc in placerat odio. Nullam porta sapien ac nunc egestas aliquam. Nulla vitae pharetra enim. Sed sed consectetur nibh, vel placerat mauris.</p>
                            </div><!-- .entry-summary -->
                        </div><!-- .tab-pane -->
                    </div><!-- .tabs-contents-wrapper -->
                </div><!-- .tabs-wrapper -->
            </div><!-- .entry-content -->
        </div><!-- .container -->
    </div><!-- #services -->

<?php } ?>
